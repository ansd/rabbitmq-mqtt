...
deploy_rabbit_and_clients(){
    TEST_ID="$1"
    RABBITMQ_IMAGE="$2"
    sed "s|{{testid}}|$TEST_ID| ; s|{{rabbitmq_image}}|$RABBITMQ_IMAGE|" mqtt-rabbit.yml | kubectl apply -f -
    sleep 20
    ...
    for podindex in $(seq "$NUM_PODS"); do
        sed "s/{{podindex}}/$podindex/ ; s/{{testid}}/$TEST_ID/" mqtt-conn.yml | kubectl apply -f -
    done
}
...
deploy_rabbit_and_clients "3-12" "rabbitmq:3.12.0-beta.2-management"
deploy_rabbit_and_clients "3-11" "rabbitmq:3.11.10-management"
